...
print("Time to extract sentences out of set:")
system.time(hold_sentences <- to_sentences(hold))
...
print("Time to preprocess hold out set:")
hold_tokens <- preprocess(hold_sentences)
...
The best evaluation method is in-vivo evaluation, but perplexity is a standard alternative.
...
model_apply <- function(test, train){
  ...
  # set NAs to zero for summation and return
  dt[is.na(mle), mle := 0]
...
test_bi_PP <- exp(-sum(log(test_bi$mle)) / length(test_bi$mle))
test_tri_PP <- exp(-sum(log(test_tri$mle)) / length(test_tri$mle))
test_four_PP <- exp(-sum(log(test_four$mle)) / length(test_four$mle))
test_five_PP <- exp(-sum(log(test_five$mle)) / length(test_five$mle))
