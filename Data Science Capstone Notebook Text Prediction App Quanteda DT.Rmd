...
training_corpus <- corpus(collection_sample)
...
create_ngrams_dt <- function(q_corpus, n) {

  corpus_dfm <- dfm(q_corpus,
                    tolower = TRUE,
                    remove_numbers = TRUE,
                    remove_punct = TRUE,
                    remove_symbols = TRUE,
  ...
  dt <- data.table(ngrams = corpus_dfm@Dimnames$features)
  
}

# QUANTEDA creating data.tables with all ngrams
...
# QUANTEDA Mixed ngrams
print("Time of preprocessing creating ngrams: Twitter")
system.time(twitter_dt <- create_ngrams_dt(twitter_corpus))
print("Time of preprocessing creating ngrams: News")
system.time(news_dt <- create_ngrams_dt(news_corpus))
print("Time of preprocessing creating ngrams: Blogs")
system.time(blogs_dt <- create_ngrams_dt(blogs_corpus))
print("Time to bind ngram data.tables: Collection")
system.time(ngrams_dt <- rbindlist(list(twitter_dt, news_dt, blogs_dt)))
